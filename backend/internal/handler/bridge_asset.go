...
type BridgeAssetHandler struct{ service service.BridgeAssetService }

func NewBridgeAssetHandler(s service.BridgeAssetService) *BridgeAssetHandler {
	return &BridgeAssetHandler{service: s}
}

func (h *BridgeAssetHandler) Register(group *gin.RouterGroup) {
	resource := group.Group("/bridges")
	resource.GET("", h.list)
...
func (h *BridgeAssetHandler) list(c *gin.Context) {
	query := bindPage(c)
...
func (h *BridgeAssetHandler) get(c *gin.Context) {
	id, ok := parseID(c)
	if !ok {
		return
	}
...
func (h *BridgeAssetHandler) create(c *gin.Context) {
	var input dto.CreateBridgeAsset
...
func (h *BridgeAssetHandler) update(c *gin.Context) {
	id, ok := parseID(c)
	if !ok {
		return
	}
	var input dto.UpdateBridgeAsset
	if err := c.ShouldBindJSON(&input); err != nil {
		util.Fail(c, http.StatusBadRequest, "invalid_request", err.Error())
		return
	}
...
func (h *BridgeAssetHandler) transition(c *gin.Context) {
	id, ok := parseID(c)
	if !ok {
		return
	}
...
func (h *BridgeAssetHandler) remove(c *gin.Context) {
