...
func New(cfg config.Config, db *gorm.DB, redisClient *redis.Client, logger *slog.Logger) *gin.Engine {
	...
	bridgeAssetService := service.NewBridgeAssetService(bridgeAssetRepository, defectFindingRepository, securityService)
	...
	priorityDecisionService := service.NewPriorityDecisionService(priorityDecisionRepository, bridgeAssetRepository, defectFindingRepository, securityService)
